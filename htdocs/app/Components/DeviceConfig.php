...
class DeviceConfig
{
	...
	public static function attendance($student_data,$deviceID,$datetime,$absent,$flag="",$timetableID,$periodName){
		
		$timetable=Timetable::where('timetable_id',$timetableID)->first();
		if(!$timetable)
			return true;
		...
		$lunch=Timetable::where('course',$student_data->course)->where('department',$student_data->department)->where('year',$student_data->current_year)->where('status',1)->where('lunchbreak',1)->first();
		...
		if($attendance){
				...
				$check_in=Attendance::where('student',$student_data->stud_id)->where('date',DATE('Y-m-d',strtotime($datetime)))->where(\DB::raw("p_".$periodName."_time"),'!=','00:00:00')->count();
				
				$lunch_period=$attendance->lunch;
				if($lunch){
				...
				if($timetable->lunchbreak==1){
					if($flag == 'manual'){
						if($absent==1)
							$qry=["p_".$periodName=>'3',"p_".$periodName."_time"=>"00:00:00","p_".$periodName."_device"=>$deviceID,'lunch'=>$lunch_period];
						else
							$qry=["p_".$periodName=>'3',"p_".$periodName."_time"=>$att_time,"p_".$periodName."_device"=>$deviceID,'lunch'=>$lunch_period];
					}elseif($check_in > 0)
						$qry=["p_out_".$periodName=>'3',"p_out_".$periodName."_time"=>$att_time,"p_out_".$periodName."_device"=>$deviceID,'lunch'=>$lunch_period];
					else
						$qry=["p_".$periodName=>'3',"p_".$periodName."_time"=>$att_time,"p_".$periodName."_device"=>$deviceID,'lunch'=>$lunch_period];
					//$att_status="lunch";
				}else{
					if($flag=="manual"){
						if($absent==1)
							$qry=["p_".$periodName=>$att,"p_".$periodName."_time"=>"00:00:00","p_".$periodName."_device"=>$deviceID,'lunch'=>$lunch_period];
						else
							$qry=["p_".$periodName=>$att,"p_".$periodName."_time"=>$att_time,"p_".$periodName."_device"=>$deviceID,'lunch'=>$lunch_period];
					}elseif($check_in > 0)
						$qry=["p_out_".$periodName=>'1',"p_out_".$periodName."_time"=>$att_time,"p_out_".$periodName."_device"=>$deviceID,'lunch'=>$lunch_period];
					else
						$qry=["p_".$periodName=>$att,"p_".$periodName."_time"=>$att_time,"p_".$periodName."_device"=>$deviceID,'lunch'=>$lunch_period];
				}
				...
				Attendance::where('attendance_id',$attendance->attendance_id)->update($qry);
					...
					$time_fld="p_".$periodName."_time";
					$device_fld="p_".$periodName."_device";
					...
					if($absent==1){
						...
						$att_status="late";
					}elseif(strtotime($punch_time) > strtotime($late_time_lmt)){ // Absent
						if($timetable->lunchbreak==1)
							$att_insert->$status_fld=3;
						else
							$att_insert->$status_fld=2;
					...
					if($lunch){
						$att_insert->lunch=$lunch->period;
					}
					$att_insert->save();
			}

			$subject="";
			$check_in=Attendance::where('student',$student_data->stud_id)->where('date',DATE('Y-m-d',strtotime($datetime)))->where(\DB::raw("p_".$periodName."_notification"),0)->count();	

			$att_percent=Attendance::where('student',$student_data->stud_id)->where('date',DATE('Y-m-d',strtotime($datetime)))->first();
			$after_lunch="p_".Common::periodinwords($lunch->period+1);
			if($att_percent->p_one > 0 && $att_percent->$after_lunch > 0)
				$att_percent->attendance=2;
			elseif($att_percent->p_one > 0 || $att_percent->$after_lunch > 0 )
				$att_percent->attendance=1;
			else
				$att_percent->attendance=0;
			...
			if($check_in == 1){
				...
				Attendance::where('student',$student_data->stud_id)->where('date',DATE('Y-m-d',strtotime($datetime)))->update(["p_".$periodName."_notification"=>1]);
			}else{
				Attendance::where('student',$student_data->stud_id)->where('date',DATE('Y-m-d',strtotime($datetime)))->update(["p_".$periodName."_notification"=>2]);
			}
