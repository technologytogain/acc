...
class StudentController extends Controller
{
    ...
    public function details(Request $request)
    {
        ...
        $data = Student::where('status', 1)->where('photo','!=','')->where('department','!=',0)->where('contact_no','!=','')->where('upgrade',0)->whereRaw($qry)->orderBy('register_no', 'ASC');
    ...
    public function registeredparentdetails(Request $request){
     ...
     public function download(Request $request){
        ...
        if($request->page=="index")
            $student = Student::where('status', 1)->where('photo','!=','')->where('department','!=',0)->where('contact_no','!=','')->where('upgrade',0)->whereRaw($qry)->orderBy('register_no', 'ASC')->get();
        elseif($request->page=="pending")
            $student = Student::whereRaw(' status=1 AND upgrade=0 AND ( photo="" OR ISNULL(photo) OR department="" OR ISNULL(department) OR contact_no="" OR ISNULL(contact_no) ) ')->whereRaw('('.$qry.')')->orderBy('register_no', 'ASC')->get();
        $dataSet=[];
        $inc=1;
