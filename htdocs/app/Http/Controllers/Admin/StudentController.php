...
class StudentController extends Controller
{
    ...
    public function details(Request $request)
    {
        ...
        if($request->year && $request->year==777)
            $qry .= ' AND upgrade=1';
        elseif($request->year && $request->year==4)
            $qry .= ' AND current_year=' . $request->year.' AND upgrade=0';
        elseif($request->year)
        ...
        $data = Student::where('status', 1)->where('photo','!=','')->where('department','!=',0)->where('contact_no','!=','')->whereRaw($qry)->orderBy('register_no', 'ASC');
        return DataTables::of($data)
            ...
            ->editColumn('current_year', function ($data) {
                ...
                if ($current_year) {
                    if($data->upgrade==1 )
                        return "<span style='color:red;font-weight:bold;'> Passed Out</span>";
                    else
            ...
            ->rawColumns(['action','photo','current_year'])
            ->make(true);
    }
    ...
    public function pendingdetails(Request $request)
    {
        ...
        $data = Student::whereRaw(' status=1 AND upgrade=0 AND ( photo="" OR ISNULL(photo) OR department="" OR ISNULL(department) OR contact_no="" OR ISNULL(contact_no) ) ')->whereRaw('('.$qry.')')->orderBy('register_no', 'ASC');
        return DataTables::of($data)
            ->addColumn('action', function ($data) {
                 $btn='';
                if(Action::chkaccess('student.edit'))
    ...
    public function failureform(Request $request){
     ...
     public function failurestore(Request $request){
        ...
        $input=$request->all();
        $input['current_year']=$request->current_year;
        $exist_acc = Student::yearfilter($input)->status(1)->where('failure',1)->pluck('stud_id')->toArray();
    ...
    public function failuredetails(){
     ...
     public function failurelist(Request $request){
        $student=Student::yearfilter($request->all())->status(1)->get();
    ...
    public function registeredparentdetails(Request $request){
     ...
     public function download(Request $request){
        ...
        if($request->page=="index")
            $student = Student::where('status', 1)->where('photo','!=','')->where('department','!=',0)->where('contact_no','!=','')->whereRaw($qry)->orderBy('register_no', 'ASC')->get();
        elseif($request->page=="pending")
            $student = Student::whereRaw(' status=1 AND ( photo="" OR ISNULL(photo) OR department="" OR ISNULL(department) OR contact_no="" OR ISNULL(contact_no) ) ')->whereRaw('('.$qry.')')->orderBy('register_no', 'ASC')->get();
        $dataSet=[];
        $inc=1;
        ...
        foreach($student as $key => $Data) {
            ...
            $dataSet[]=[$inc,$Data->register_no,$Data->name,$course,$department,$year,$Data->father_name,$dob,$gender,$Data->contact_no,$Data->parent_contactno,$Data->parent_contactno2,$blood_group,$Data->address];
    ...
    public function templatewithdata(Request $request){
        ...
        $student=Student::where('course',$request->course)->where('department',$request->department)->where('academic_year',$request->academic_year)->where('current_year',$request->current_year)->where('status',1)->where('upgrade',0)->get();
