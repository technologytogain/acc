...
class DailyAttendance extends Command{
	...
	public function handle(){
		...
		if($this->option('datetime') ==true){
			...
			$period_qry=Timetable::whereRaw(' ADDTIME(from_time,"0:'.$settings->attendance_interval.':0") ="'.DATE("H:i:00",strtotime($datetime)).'" ')->where('weekday',DATE('N'))->where('status',1)->get();
			...
			$period_qry=Timetable::whereRaw(' ADDTIME(from_time,"0:'.$settings->attendance_interval.':0") ="'.DATE("H:i:00").'" ')->where('weekday',DATE('N'))->where('status',1)->get();
		}
		...
		$yearSet=[];
		foreach($period_qry as $years_timetable){
			$yearSet[]=$years_timetable->year;
		}
		
		Student::where('status',1)->whereIn('current_year',$yearSet)->orderBy('stud_id', 'ASC')->chunk(1, function ($student_qry)use($datetime,$settings) {
